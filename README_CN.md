...
>
> 🪶 **极致轻量——零依赖，零锁定。** 整个系统就是纯 Markdown 文件。没有框架要学、没有数据库要维护、没有 Docker 要配、没有守护进程要看管。每个 skill 就是一个 `SKILL.md`，任何 LLM 都能读懂——换成 [Codex CLI](skills/skills-codex/)、[GitHub Copilot CLI](docs/COPILOT_ADAPTATION.md)、[OpenClaw](docs/OPENCLAW_ADAPTATION.md)、[Cursor](docs/CURSOR_ADAPTATION.md)、[Trae](docs/TRAE_ARIS_RUNBOOK_CN.md)、[Antigravity](docs/ANTIGRAVITY_ADAPTATION_CN.md)、Windsurf 或者你自己的 agent，工作流照样跑。Fork 它、改写它、适配到你的技术栈。
>
...
🌐 **外部项目 & 文档（10 个）：** [open-source-hardening-skills](https://github.com/zeyuzhangzyz/open-source-hardening-skills) · [CitationClaw](https://github.com/VisionXLab/CitationClaw) · [Antigravity 适配指南](docs/ANTIGRAVITY_ADAPTATION_CN.md) · [GitHub Copilot CLI 适配指南](docs/COPILOT_ADAPTATION.md) · [OpenClaw 适配指南](docs/OPENCLAW_ADAPTATION.md) · [Cursor 适配指南](docs/CURSOR_ADAPTATION.md) · [Trae 适配指南](docs/TRAE_ARIS_RUNBOOK_CN.md) · [`skills-codex`](skills/skills-codex/) · [paper-illustration](skills/paper-illustration/SKILL.md) · [auto-hparam-tuning](https://github.com/zxh0916/auto-hparam-tuning)
...
| 💻 [GitHub Copilot CLI 适配指南](docs/COPILOT_ADAPTATION.md) | 通用 | 在 GitHub Copilot CLI 中使用 ARIS skills——原生 `SKILL.md` skills、`/skills add`、`/mcp add`、仓库指令、状态文件恢复（英文指南） |
